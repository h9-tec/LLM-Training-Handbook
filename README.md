...
**Who it's for:** ML engineers, infra engineers, and technical leads who train, fine-tune, or adapt LLMs, or who need to read training reports critically. Interview candidates for training/infra roles will find every classic question here, answered with real numbers.
...
Every subsequent chapter tells you which row it applies to.
...
**SmolLM3-3B** cost roughly $1-2M at market rates (384 H100s for ~24-30 days) for an 11T-token run, and the team documents that a disciplined follower using their published lessons, spot capacity, and smaller ablation budgets could land a competitive 3B model well under $100K of marginal mistakes avoided. The expensive part of training is not the happy path; it is the restarts, and Ch. 19 catalogs them.
...
2. **Restarts and instability.** PaLM's team handled around 20 loss spikes by rewinding and skipping batches; a published 65B run reported roughly an extra month and ~130 MWh spent handling spikes; SmolLM3 restarted from scratch after burning 1T tokens on a seed bug. Effective training time on Llama 3.1 405B was above 90%, which is considered excellent, and still means paying for 16,384 H100s during the other ~10%.
...
**Language identification.** A fastText-style classifier with a threshold (FineWeb keeps documents with P(English) ≥ 0.65). For Arabic corpora the equivalent step must additionally decide what to do with dialects, romanized Arabic (Arabizi), and heavy code-switching, which off-the-shelf LID handles poorly; ALLaM's team built their own filtering to curate 500B Arabic tokens (Ch. 13).

**Quality filtering.** FineWeb layers C4-style and MassiveText-style heuristics plus 50+ custom metric-threshold pairs (duplicate-line fraction, terminal-punctuation frequency, symbol ratios, and so on), and, critically, **ablated each filter** instead of trusting intuition. They found C4's rule dropping lines without terminal punctuation too aggressive: it removed good content along with bad. Lesson two: every filter is a hypothesis; the only proof is training a model with and without it.
...
High fertility on your language means, simultaneously: fewer effective words per context window, more tokens (thus more compute and more money) to represent the same corpus, higher serving cost per answer, and worse per-token learning signal. The ALLaM report shows fertility comparisons directly: an English-centric tokenizer's fertility on Arabic is roughly double that of their Arabic-aware tokenizer, meaning the same Arabic dataset costs about twice the tokens to train through. When you read "trained on X trillion tokens," always ask: whose tokens?
...
2. **Vocabulary expansion of an existing model** (the adaptation path): merge new-language tokens into a pretrained model's tokenizer, extend the embedding matrix, and initialize each new token's embedding as the average of the old-tokenizer subtokens that compose it (the WECHSEL-style initialization used by ALLaM, Kuwain, and RightNow-Arabic). This is now the standard first move of language adaptation and is covered operationally in Ch. 13.
...
- **Same size, fewer tokens.** SmolLM3 ran its ablations as the full 3B architecture on 100B tokens (roughly 1% of the final run), with architecture screens on a 1B proxy at 45B tokens. Cheap enough to iterate, big enough to transfer.
...
**Multi-token prediction (MTP) is a signal-density decision.** DeepSeek-V3 trains auxiliary heads to predict several future tokens, extracting more gradient signal per sequence (and doubling as a speculative-decoding module at inference). More learning per token processed = fewer GPU-hours per unit of capability.
...
- **The cost when unmanaged.** One published 65B run reported ~30 extra days and ~129 MWh spent on spike handling (rewinds, batch skips, LR surgery). Instability is a budget line, not an anecdote.
...
Lower precision doubles arithmetic throughput and halves memory traffic per step down, so every generation of training has pushed one format lower, and every push has produced a characteristic class of failures. The three-era history:
...
### 10.1 The four axes, as one mental model

Training parallelism is four nested answers to "what do we split?":
...
- **Expert parallelism (EP):** MoE's fifth axis: experts distributed across GPUs, tokens routed via all-to-all. Combines with all of the above and is the dominant communication cost in large MoE training.
...
- **Checkpointing math.** Time-to-write scales with state size (Sec. 9.2) over storage bandwidth; if it stalls training, you either checkpoint less often (raising expected lost work per failure, Ch. 11) or fix I/O. SmolLM3's team hit AWS FSx-for-Lustre failures mid-project and moved hot checkpoint I/O to node-local RAID-0 NVMe with GPUDirect Storage, keeping cold data on S3: a three-tier pattern worth copying.
...
Over a 54-day pretraining window on 16,384 H100s, Meta recorded **466 job interruptions**: 47 planned (maintenance) and **419 unexpected**, i.e. one unexpected failure roughly every three hours. The breakdown of the 419: about 58.7% GPU-related in total, including 148 (30.1%) GPU/NVLink faults and 72 (17.2%) HBM3 memory failures, plus GPU SRAM (4.5%) and GPU system-processor (4.1%) issues; network switches/cables contributed ~8.4%; and exactly **2 CPU failures** in 54 days. The asymmetry is the point: a 700W accelerator under thermal stress fails orders of magnitude more often than a CPU. Despite all this, only 3 incidents needed significant manual intervention (the rest handled by automation) and effective training time stayed above 90%. Two more operational details worth knowing because they surprise people: diurnal temperature swings alone moved throughput 1-2%, and the synchronized power draw of 16K GPUs swings tens of megawatts, enough to stress a data center's grid interface.

Scale that failure rate mentally: clusters of 100K+ GPUs at the same per-component rates see failures many times per hour, which is why fault tolerance, not peak FLOPs, is the actual frontier of training infrastructure. Independent datapoints agree on magnitude: a published H200 cluster of ~1,600 GPUs logged ~205 failures over four months.
...
- **Watch for silent data corruption.** Rare flipped bits in accelerators can corrupt training without crashing anything; periodic tensor checks are the defense, with a real throughput trade documented by MegaScale-Omni's team (they narrowed checks to encoder outputs after full-tensor checking hurt throughput too much).
...
**Path B: continued pretraining of a strong open base (AceGPT, 2023; ALLaM 7B-continued; the default 2026 answer).** AceGPT continued Llama-2 (7B/13B) on Arabic-dominant mixtures of 30B/10B tokens respectively, then localized post-training (Arabic instructions, RLAIF for cultural alignment): a fraction of from-scratch cost for most of the benefit. ALLaM's team ran the definitive controlled study: they first adapted Llama-2 via tokenizer/vocabulary expansion plus 1.2T tokens of mixed Arabic-English continued pretraining, *then* applied the learned recipe from scratch, and reported the from-scratch 7B beating the continued 7B. Read carefully, that result says: adaptation is the fast, cheap 90%; from-scratch is the expensive last 10%, worth it mainly at national-program budgets. For everyone else, Path B on a 2026-class base (Qwen3, Llama, Gemma) starts from a far stronger prior than Llama-2 ever offered.

**Path C: surgical small-model adaptation (Kuwain, 2025; RightNow-Arabic-0.5B, 2026).** The budget tier, and the most instructive for teams with single-node clusters. Kuwain-1.5B injected an Arabic vocabulary into TinyLlama-1.1B and reported ~8% average Arabic improvement while preserving English. RightNow-Arabic-0.5B-Turbo documents the full modern micro-recipe: inject 27,032 Arabic tokens into Qwen2.5-0.5B, continue pretraining on ~504M Arabic tokens, SFT with response-only loss masking, DPO, then a weight-soup merge of variants, exported to GGUF for edge (398MB quantized, 635 tok/s on one H100 at batch 1, phone-capable). Total compute: a single 8×H100 node and days, not months. This tier exists because of everything Paths A/B learned; it is the trickle-down.
...
1. **Train an Arabic (or merge a bilingual) tokenizer** and select which new tokens to add. More tokens = better fertility but a bigger embedding matrix; at 0.5B scale RightNow chose ~27K tokens as the sweet spot, while ALLaM merged a full Arabic tokenizer into Llama-2's, cutting Arabic fertility roughly in half (Ch. 4) and thereby halving the token cost of every subsequent Arabic training and inference step.
2. **Extend the embedding and output matrices**, initializing each new token's vectors from the mean of the embeddings of its old-tokenizer decomposition (tokenize the new token's string with the *old* tokenizer, average those pieces): the WECHSEL-style initialization ALLaM, Kuwain, and RightNow all converge on. Random init here measurably slows adaptation; you would be forcing the model to relearn what it already knows under new names.
3. **Continue pretraining on a mixed corpus, never a pure one.** The forgetting problem is real and the antidote is *anchoring*: ALLaM argues explicitly that vocabulary expansion paired with continued English presence in the mixture is what prevents catastrophic forgetting of the base model's abilities. Ratios in the record run from Arabic-dominant-with-English-anchor (AceGPT) to ~1:1 blends; the honest answer is that your replay ratio is an ablation (Sec. 5.3), with 10-30% original-distribution data as the common starting band in the broader continued-pretraining literature.
4. **Handle the learning rate conservatively.** ALLaM's reported finding: simply continuing at the base model's (late, low) learning rate was often *more* effective than elaborate re-warmup schemes; aggressive re-warming of the LR is a classic way to spike loss on a healthy checkpoint (the mechanisms of Ch. 8 all apply, since you are perturbing a converged optimizer landscape). Start low, ablate upward only with evidence.
...
| Team with 1-2 nodes | C: token injection + hundreds of millions to a few billion tokens + SFT/DPO | Kuwain, RightNow-Arabic |
...
**Quality beats quantity, with a caveat.** The LIMA-era result (a thousand excellent examples can set style/format) survives as a half-truth: format and tone are cheap to teach, but *capabilities* under SFT scale with high-quality coverage of the skill (Tulu's math and coding gains came from targeted volume, not vibes). Filtering with LLM judges is now standard plumbing: e.g., the Trillion-7B report scores candidate responses with a strong open model and keeps only high scorers, a pattern repeated across 2025-2026 reports.
...
3. **Rejection-sampling SFT** (~800K: 600K verified reasoning + 200K general, Sec. 14): re-broadens the model beyond math/code.
...
### 19.1 Every documented incident in this book, one table
...
| A published 65B run | ~30 extra days, ~129 MWh on spike handling | Unmitigated spike mechanisms | Motivation for adaptive clipping (ZClip) and the modern package (Ch. 8) |
| Llama 3.1 405B | 419 unexpected interruptions in 54 days (one per ~3h); 58.7% GPU-related; 2 CPU failures; >90% effective time | H100/HBM3 thermal-stress failure physics at 16K-GPU scale | Automation-first ops (3 manual interventions total); checkpoint cadence; power/thermal engineering (Ch. 11) |
...
| SmolLM3 3B | Restarted after 1T tokens: model underperformed its *smaller* predecessor | Same RNG seed across all tensor-parallel ranks | Per-rank seeding; determinism tests; plus FSx storage failure → local NVMe + GPUDirect (Ch. 10) |
...
**Full-run chronicles and reliability.** OPT: Open Pre-trained Transformer LMs (arXiv:2205.01068) and the OPT-175B logbook/chronicles in facebookresearch/metaseq · The Llama 3 Herd of Models (arXiv:2407.21783) · BLOOM (arXiv:2211.05100) and the BigScience training notes · MegaScale (arXiv:2402.15627, NSDI'24) · Revisiting Reliability in Large-Scale ML Research Clusters (Meta, HPCA'25) · The Smol Training Playbook + SmolLM3 blog (Hugging Face, 2025)
...
**Stability literature.** GLM-130B (arXiv:2210.02414) · PaLM (arXiv:2204.02311) · Spike No More (Takase et al., arXiv:2312.16903) · A Theory on Adam Instability (Molybog et al., arXiv:2304.09871) · ZClip (arXiv:2504.02507)
...
**Post-training.** Tulu 3 (arXiv:2411.15124) and the Tulu-3-405B report · DeepSeek-R1 (arXiv:2501.12948) · DeepSeekMath / GRPO (arXiv:2402.03300) · DPO (arXiv:2305.18290) · OpenAI's two GPT-4o sycophancy postmortems (openai.com, April-May 2025) · LoRA Learns Less and Forgets Less (arXiv:2405.09673) · LoRA vs Full Fine-tuning: An Illusion of Equivalence (arXiv:2410.21228) · LoRA Without Regret (Thinking Machines, 2025)

**Arabic and language adaptation.** Jais (arXiv:2308.16149) and the Jais family report (arXiv:2409.12186) · AceGPT (arXiv:2309.12053) · ALLaM (arXiv:2407.15390) · Kuwain-1.5B (Misraj, 2025) · RightNow-Arabic-0.5B-Turbo (2026) · Fanar (arXiv:2501.13944) and MorphBPE · Atlas-Chat (arXiv:2409.17912) · WECHSEL (arXiv:2112.06598)

---

*Descriptions and numbers were compiled from the cited reports as of late 2026. If a figure has drifted or a claim needs a correction, open an issue or a PR: this handbook is meant to be argued with.*
